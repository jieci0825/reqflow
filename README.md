...
[![npm version](https://img.shields.io/npm/v/reqflow.svg)](https://www.npmjs.com/package/reqflow)
[![license](https://img.shields.io/npm/l/reqflow.svg)](https://github.com/jieci0825/reqflow/blob/main/LICENSE)
...
reqflow 按功能拆分为多个子路径，按需导入：

| 路径 | 说明 |
| --- | --- |
| `reqflow` | 核心：`createRequest`、`compose` 及所有类型定义 |
| `reqflow/adapters/fetch` | 基于 Fetch API 的传输层适配器 |
| `reqflow/plugins` | 全部内置插件 |
| `reqflow/sse` | SSE / NDJSON 流式解析工具 |
...
// POST
await http.post('/users', { name: 'reqflow' })
...
const http = createRequest({
  ...
  headers: { 'X-App': 'reqflow' },     // 可选，所有请求的默认请求头
...
await http.request({
  ...
  data: { name: 'reqflow' },         // 请求体（POST/PUT/PATCH）
  timeout: 3000,
...
```typescript
import type { Middleware } from 'reqflow'
...
```typescript
import type { Adapter, RequestConfig, Response } from 'reqflow'
...
```typescript
import type { Plugin } from 'reqflow'
...
```typescript
import { tokenPlugin } from 'reqflow/plugins'
...
```typescript
import { dualTokenPlugin } from 'reqflow/plugins'
...
```typescript
import { errorPlugin } from 'reqflow/plugins'
...
```typescript
import { retryPlugin } from 'reqflow/plugins'
...
```typescript
import { cachePlugin } from 'reqflow/plugins'
...
```typescript
import { dedupPlugin } from 'reqflow/plugins'
...
```typescript
import { loadingPlugin } from 'reqflow/plugins'
...
```typescript
import { racePlugin } from 'reqflow/plugins'
